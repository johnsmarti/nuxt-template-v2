<template>
  <div class="field relative mb-4">
    <div class="input-wrapper relative">
      <Password unstyled :pt="theme" :ptOptions="{
        mergeProps: ptViewMerge
      }" v-model="value" :placeholder="placeholder" :feedback="false" toggleMask>
        <template #maskicon="{ toggleCallback }">
          <EyeSlashIcon @click="toggleCallback"
            class="end-3 text-surface-500 absolute top-1/2 -mt-2 w-4 h-4 cursor-pointer" />
        </template>
        <template #unmaskicon="{ toggleCallback }">
          <EyeIcon @click="toggleCallback"
            class="end-3 text-surface-500 absolute top-1/2 -mt-2 w-4 h-4 cursor-pointer" />
        </template>
      </Password>
      <label for="value"

  ...
  <AuthLayout>
    ...
    <template #right>
      <div class="mt-16 w-full max-w-md">
        ...
        <Form :resolver="passwordRequestResolver" @submit="onSubmit" class="flex flex-col gap-4 w-full max-w-md">
          <FormField v-slot="$field" name="email">
            <InputText v-model="$field.value" label="Email" placeholder="email@exemplo.com" />
          </FormField>
          <ButtonPrimary label="Enviar" class="w-full !border-blue-600 !bg-blue-600 hover:!bg-blue-700"
            @click="onSubmitClick" />
        </Form>
      </div>
    </template>
...
import { passwordRequestResolver } from "@/schemas/auth";
...
const onSubmitClick = () => mutate({ email: email.value });
